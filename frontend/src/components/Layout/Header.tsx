...
export default function Header() {
  ...
  return (
    <header className="bg-gray-900 text-white px-4 py-3 flex items-center justify-between border-b border-gray-700">
      <Link to="/" className="font-semibold text-emerald-400">SecureMail</Link>
      <button aria-expanded={open} aria-label="Открыть меню" className="p-2 rounded hover:bg-dark-800 transition-colors" onClick={() => setOpen(!open)}>
        <span className="block w-6 h-0.5 bg-emerald-500 mb-1"></span>
        <span className="block w-6 h-0.5 bg-emerald-500 mb-1"></span>
        <span className="block w-6 h-0.5 bg-emerald-500"></span>
      </button>
      {open && (
        <nav className="absolute right-4 top-14 bg-gray-800/95 backdrop-blur rounded shadow-soft w-56 border border-gray-700">
          <ul className="py-2">
